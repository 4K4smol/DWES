    ...
    <div class="container mx-auto flex px-8 xl:px-0">
        ...
        <div id="menu"
            ...
            <div class="flex flex-col text-center lg:flex-row">
                <?php if (!Auth::check()): ?>
                <a href="{{ route('login') }}"
                    class="bg-transparent hover:bg-blue-500 text-black-700 font-semibold hover:text-white py-2 px-4 border border-blue-500 hover:border-transparent">
                    Iniciar Sesión</a>
                ...
                <form method="POST" action="{{ route('logout') }}" id="logout-form">
                    @csrf
                    <a href="#" class="bg-transparent hover:bg-blue-500 text-black-700 font-semibold hover:text-white py-2 px-4 border border-blue-500 hover:border-transparent"
                        onclick="event.preventDefault(); document.getElementById('logout-form').submit();">
                        Cerrar sesión
                    </a>
                </form>
                <p style="font-size: 24px; font-weight: bold; color: #bd0707; margin-left:20px;">
                    <?= Auth::user()->name ?></p>
                <?php endif; ?>
            </div>
        </div>
