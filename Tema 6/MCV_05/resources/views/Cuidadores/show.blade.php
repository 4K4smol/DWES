...
@section('titulo', $cuidador->nombre . ' - Zooilogico.es')
...
@section('contenido-dentro')
    <div class="max-w-4xl mx-auto bg-white dark:bg-gray-800 rounded-xl shadow-2xl hover:shadow-xl transition-shadow duration-300 overflow-hidden">
        <div class="relative group">
            <a href="{{ route('animals.view', ['animal' => $animal]) }}"
        ...
        <div class="p-8 space-y-8">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 text-gray-700 dark:text-gray-300">
                <div class="space-y-3">
                    @if ($cuidador->titulacion1)
                        <div class="flex items-center gap-2">
                            <p>
                                <strong class="text-purple-600 dark:text-purple-400">Titulación 1:</strong>
                                <a href="{{ route('titulaciones.show', $cuidador->titulacion1) }}">{{ $cuidador->titulacion1->nombre }}</a>
                            </p>
                        </div>
                    @endif

                    @if ($cuidador->titulacion2)
                        <div class="flex items-center gap-2">
                            <p>
                                <strong class="text-purple-600 dark:text-purple-400">Titulación 2:</strong>
                                <a href="{{ route('titulaciones.show', $cuidador->titulacion2) }}">{{ $cuidador->titulacion2->nombre }}</a>
                            </p>
                        </div>
                    @endif
                </div>
